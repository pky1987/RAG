...
def display_splash_screen(args: argparse.Namespace) -> None:
    ...
    # Determine whether WebUI static assets are present so we can show the correct URL
    webui_static_dir = Path(__file__).parent / "webui"
    webui_suffix = "/webui" if webui_static_dir.exists() else ""
    if args.host == "0.0.0.0":
        ASCIIColors.magenta("\n🌐 Server Access Information:")
        ASCIIColors.white("    ├─ WebUI (local): ", end="")
        ASCIIColors.yellow(f"{protocol}://localhost:{args.port}{webui_suffix}")
        ASCIIColors.white("    ├─ Remote Access: ", end="")
        ASCIIColors.yellow(f"{protocol}://<your-ip-address>:{args.port}{webui_suffix}")
    ...
    """)
        ...
        ASCIIColors.magenta("\n🌐 Server Access Information:")
        ASCIIColors.white("    ├─ WebUI (local): ", end="")
        ASCIIColors.yellow(f"{base_url}{webui_suffix}")
